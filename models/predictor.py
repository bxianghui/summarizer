...
def build_predictor(args, tokenizer, symbols, model, logger=None):
    ...
    translator = Z_Translator(args, model, tokenizer, symbols, global_scorer=scorer, logger=logger)
    return translator


class Z_Translator(object):
    """
    ...
    def __init__(self,
        ...
        if COPY:
            self.copy_generator = self.model.copy_generator
        self.vocab = vocab
        self.symbols = symbols
        self.start_token = symbols['BOS'] #+100
        self.end_token = symbols['EOS'] #+100
    ...
    def _fast_translate_batch(self,
        ...
        #decoder_outputs, state = self.decoder(tgt[:, :-1], top_vec, z_top_vec, dec_state)
        ...
        z_top_vec = self.model.bert(z, z_segs, mask_z)

        # here
        if NO_LAYER:
            src_features = top_vec
            z_src_features = z_top_vec
            ...
            if CROSS:
                ...
                top_vec1, _ = self.model.f1(top_vec, z_top_vec, z_pad_mask, src_pad_mask)
                src_pad_mask = mask_src.unsqueeze(1).expand(src_batch, z_len, src_len)
                z_pad_mask = mask_z.unsqueeze(1).expand(z_batch, z_len, z_len)
                z_top_vec1, _ = self.model.f2(z_top_vec, top_vec, src_pad_mask, z_pad_mask)

                dec_states = self.model.decoder.init_decoder_state(src, top_vec1, z, z_top_vec1)
                src_features = top_vec1
                z_src_features = z_top_vec1
            #print(src_features.size())
            #print(z_src_features.size())
            # here
        ...
        z_src_features = tile(z_src_features, beam_size, dim=0)
        ...
        for step in range(max_length):
            ...
            #dec_out, dec_states = self.model.decoder(decoder_input, src_features, z_src_features, dec_states,
            #                                         step=step)
            #def forward(self, inputs, memory_bank, z_memory_bank, src_pad_mask, tgt_pad_mask, z_pad_mask,
            dec_out, dec_states, copy_probs = self.model.decoder(decoder_input, src_features, z_src_features, dec_states,
                                                     step=step)

            # Generator forward.
            log_probs = self.generator.forward(dec_out.transpose(0,1).squeeze(0))
            if COPY:
                attn_prob, y_emb, copy_state = copy_probs
                z_copy = z_src_features[:, 0, :]
                #z_copy = z_copy.transpose(0,1).squeeze(0)
                z_copy = z_copy #.transpose(0,1).squeeze(0)
                copy_prob = self.copy_generator(torch.cat([copy_state.transpose(0,1).squeeze(0), y_emb.transpose(0,1).squeeze(0), z_copy], -1))
                #copy_prob = self.copy_generator(torch.cat([dec_out.transpose(0,1).squeeze(0), z_copy], -1))
                log_probs = copy_prob * torch.log(attn_prob.squeeze(1)+1e-9) + (1.-copy_prob)*log_probs
                #print(f'copy_prob')
                #print(copy_prob)

                #copy_prob = self.copy_generator.forward(dec_out.transpose(0,1).squeeze(0))
                #log_probs = copy_prob * torch.log(copy_probs+1e-9) + (1.-copy_prob)*log_probs
